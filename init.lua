...
local cozy = {positions = {}, mods = {"mcl_title"}}
...
local function show_status(player, message)
	for _, mod in ipairs(cozy.mods) do
		if minetest.get_modpath(mod) then
			return mod == "mcl_title" and mcl_title.set(player, "actionbar", {text = message or S("Move to stand up"), color = "white", stay = 60})
		end
	end
...
local function handle_action(player, name, action)
	...
	if mcl_player.player_attached[name] then reset_player(player, name) send_action(name, "stand")
	else
		...
		mcl_player.player_attached[name] = true mcl_player.player_set_animation(player, action, action == "sit" and 30 or 0)
...
minetest.register_globalstep(function()
	for _, player in ipairs(minetest.get_connected_players()) do
		...
		if mcl_player.player_attached[name] and not player:get_attach() and (ctrl.up or ctrl.down or ctrl.left or ctrl.right or ctrl.jump or ctrl.sneak) then
